/**
 * Fichier de définition du plugin jEasyQuizz pour jQuery
 *
 * 4 types d'exercice :
 *   - sélection de zone
 *   - drag & drop
...
(function($)
{
	$.fn.jEasyQuizz = function(options)
	{
		/**
         * default parameters
         */
							...
							for(i; i<len; i++)
							{
								switch(source[i].getAttribute('type'))
								{
									...
									case 'PicturePicking' : case 'pp' :
										exercises.push(new PicturePicking(source[i]));
										total_questions += exercises[i].getQuestions().length;

										break;
									case 2 : case '2' :
									case 'MissingWord' : case 'mw' :
										exercises.push(new Exercise(source[i]));
										total_questions += exercises[i].getQuestions().length;
									...
									case 'MultipleAnswers' : case 'ma' :
										exercises.push(new MultipleAnswers(source[i]));
										total_questions += exercises[i].getQuestions().length;
										break;
									case 5 : case '5' : case 'dd' :
									case 'DragAndDrop' : case 'dnd' :
										exercises.push(new DragAndDrop(source[i]));
										total_questions += exercises[i].getQuestions().length;
										break;
								}
							}
